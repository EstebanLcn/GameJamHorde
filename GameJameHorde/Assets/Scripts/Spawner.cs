  ...
  void Update()
  {
    ...
    if (_timeBetweenSpawn <= 0)
    {
     spawnCount++;
            ...
            {
                _objectToBeDestroyed = Instantiate(_spawner, new Vector2(transform.position.x, Random.Range(transform.position.y - 2.7f, transform.position.y + 2.7f)), Quaternion.identity);
            }
